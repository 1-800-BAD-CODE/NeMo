...
class ConcatMapDataset(Dataset):
    ...
    def __init__(
        ...
        if self.sampling_technique == "round-robin":
            ...
            dataset_ids_queue = self.np_rng.choice(a=choices, p=p, size=5000)
            dataset_id_index = 0
            while len(exhausted_datasets) < len(self.datasets):
                dataset_id = dataset_ids_queue[dataset_id_index]
                dataset_id_index += 1
                if dataset_id_index == dataset_ids_queue.shape[0]:
                    dataset_ids_queue = self.np_rng.choice(a=choices, p=p, size=5000)
                    dataset_id_index = 0
